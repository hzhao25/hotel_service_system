        ...
        <el-table-column label="操作" width="200" align="center">
          <template slot-scope="scope">
            <el-button
              ...
              size="small"
              class="blueBug"
              @click="addProductHandle(scope.row.id, scope.row.username)"
            >
              修改
            </el-button>
            <el-button
...
import { getProductList, enableOrDisableProduct, deleProduct } from '@/api/product'
...
export default class extends Vue {
  ...
  private productStatus: string | null = null; // 新增：存储选择的售卖状态
  private saleStatus = [
    { label: '启用', value: '1' },
    { label: '禁用', value: '0' }
  ]; // 新增：售卖状态列表
  ...
  private async init(isSearch?: boolean) {
    ...
    const params = {
      ...
      status: this.productStatus ? this.productStatus : undefined // 新增：添加售卖状态参数
    }
  ...
  // 添加
  private addProductHandle(st: string, username: string) {
    if (st === 'add') { //添加商品信息
      this.$router.push({ path: '/product/add' })
    } else { //修改商品信息
      this.$router.push({ path: '/product/add', query: { id: st } })
    }
  }
  ...
  private statusHandle(row: any) {
    ...
    this.$confirm('确认调整该商品的状态?', '提示', {
      ...
      enableOrDisableProduct({ id: this.id, status: !this.status ? 1 : 0 })
        .then((res) => {
...
.button-inline {
  ...
  margin-right: 5px; /* 可以根据需要调整按钮之间的间距 */
}
</style>
